...
<form action="?page={$page}&action={$action}&id={$order->id}" method="post">
    <table width="100%">
        ...
        <tr>
            ...
            <td class="ttovar">{$order->guestInfo} {*<input name="data[date]" value="{$order->date|date_format:"%d.%m.%Y"}" readonly="readonly" />*}</td>
        </tr>
        <tr>
    ...
    <table width="100%">
        ...
        {foreach from=$order->productList item=product}
            <tr>
                ...
                <td class="ttovar">{if $product.tone!==0}{$product.tone->title}{/if}</td>
