...
<form action="?page={$page}&action={$action}{if $action=='edit_product'}&id={$product->id}{/if}&rubric={$cur_rubric->getId()}" method="post" enctype="multipart/form-data">
    <table width="100%">
        ...
        <tr>
            <td class="ttovar">Горячие предложение</td>
            <td class="ttovar"><input type="checkbox" name="data[is_spec]" {if $product->isSpec}checked="checked"{/if} /></td>
        </tr>
    </table>
    <input id="save" name="save" type="submit" value="Сохранить"/>
</form>
    ...
    <form action="?page={$page}&action={$action}{if $action=='edit_product'}&id={$product->id}{/if}&rubric={$cur_rubric->getId()}" method="post" enctype="multipart/form-data">
        ...
        <table>
            {if $productToneList !== false}
                {foreach from=$productToneList item=tone}
                    <tr>
                        <td class="ttovar">Тон {$tone->title}</td>
                        <td class="ttovar"><img src="{$siteurl}files/{$tone->img->getName()}"/></td>
                        <td class="tedit"><a href="?page={$page}&action=edit_product&id={$product->id}&rubric={$cur_rubric->getId()}&delete_tone={$tone->id}" style="color: #830000">удалить</a></td>
                    </tr>
                {/foreach}
            {/if}

            <tr>
                <td class="ttovar">Тон: <input type="text" name="tone[title]" value="" style="width: 50px;"/></td>
                <td class="ttovar">Рисунок: <input type="file" name="tone" style="width: 300px;"/></td>
                <td></td>
            </tr>
        </table>
        <input id="save" name="save" type="submit" value="Добавить тон"/>
    </form>
    {/if}
    ...
    <table width="100%">
        <tr>
            <td colspan="3" style="background-color:#f7f7f7; padding: 10px; text-align:center;" valign="middle"><a href="?page={$page}&action=add_rubric&rubric={$cur_rubric->getId()}">добавить рубрику</a></td>
        </tr>
...
<table width="100%">
    <tr>
        <td colspan="5" style="background-color:#f7f7f7; padding: 10px; text-align:center;" valign="middle"><a href="?page={$page}&action=add_product&rubric={$cur_rubric->getId()}">добавить товар</a></td>
    </tr>
    {if $product_list}
